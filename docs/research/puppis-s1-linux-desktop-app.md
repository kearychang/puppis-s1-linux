...
| Linux network device | `enx<redacted>`, up, carrier present, MTU 1500 | Stable detection can combine udev properties, interface path, and behavioral checks. |
...
| DHCP/neighbors | Puppis hostname `P1411`, MAC `[REDACTED]`, appeared at a redacted lease address and `.254`; a phone client address was redacted | The observed address and `.254` are owned or proxied by the S1. The client list can be correlated with DHCP data when permissions allow. |
| Local services | TCP/53 was open on the redacted observed address and `.254`; ports 22, 80, 443, 554, 1900, 5000, 8000, 8080, 8443, and 8888 were closed | The S1 provides or proxies DNS, but no conventional web-management service was found on the bounded scan. This does not rule out UDP or an untested proprietary port. |
